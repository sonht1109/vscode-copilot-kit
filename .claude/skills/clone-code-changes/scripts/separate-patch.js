...
async function separatePatch(nowValue) {
  if (!nowValue) {
    throw new Error("Code changes number is required as the first argument.");
  }

  const patchFileDir = `${process.cwd()}/notes/code-changes/${nowValue}/diff.patch`;

  if (!fs.existsSync(patchFileDir)) {
    throw new Error(
      `Patch file for code changes #${nowValue} does not exist at path: ${patchFileDir}`,
    );
  }

  // create dir
  const prDir = `${process.cwd()}/notes/code-changes/${nowValue}/contents`;
  ...
  for await (const line of rlInterface) {
    if (line.startsWith("diff --git")) {
      ...
      filePatchStream = fs.createWriteStream(
        `${process.cwd()}/notes/code-changes/${nowValue}/contents/${lastFilePath.replace(
          /\//g,
          "_",
        )}-patch.patch`,
      );
...
if (require.main === module) {
  ...
  separatePatch(fileDir).catch((err) => {
