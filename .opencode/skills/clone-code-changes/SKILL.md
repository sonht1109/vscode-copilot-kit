...
- now: The current timestamp in the format YYYYMMDD_HHmmss. Invent this value at runtime by running:
...
- cwd: The current working directory of the local repository where the user is running the commands (not `.opencode` directory). Must use this cwd for all file paths in the steps below.
...
```bash
node <cwd>/.opencode/skills/clone-code-changes/scripts/separate-patch.js <now>
```
